  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <div className="h-[400px] border-b">
        <MapLoader markers={markers} zoom={9} height="400px" />
      </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        {markers.length === 0 && (
          <div className="text-center py-16">
            ...
            <Link href="/buy" className="px-6 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800">
            ...
            <Link key={listing.id} href={`/listing/${listing.id}`}
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-1">
                  ...
                  {listing.is_verified && (
                    <span className="ml-2 shrink-0 px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded-full font-medium">✓ Verified</span>
                  )}
                </div>
                <p className="text-lg font-bold text-green-800">
                  ₹{Number(listing.price).toLocaleString("en-IN")}
                  <span className="text-sm font-normal text-gray-500 ml-1">
                    {listing.price_basis === "per_acre" ? "/ acre" : listing.price_basis === "per_guntha" ? "/ guntha" : "total"}
                  </span>
                </p>
                <p className="text-sm text-gray-500">
                  {listing.area_value} {listing.area_unit} · {listing.land_type?.replace(/_/g, " ")} · {[listing.village, listing.taluka, listing.district].filter(Boolean).join(", ")}
                </p>
              </div>
            </Link>
          ))}
