...
export default function SearchFilters() {
  ...
  return (
    <div className="flex flex-wrap gap-3 px-6 py-3 bg-gray-50 border-b">
      <select defaultValue={params.get("land_type") ?? ""} onChange={(e) => set("land_type", e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm outline-none bg-white">
      ...
      <select defaultValue={params.get("max_price") ?? ""} onChange={(e) => set("max_price", e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm outline-none bg-white">
      ...
      <select defaultValue={params.get("max_area") ?? ""} onChange={(e) => set("max_area", e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm outline-none bg-white">
      ...
      <select defaultValue={params.get("verified") ?? ""} onChange={(e) => set("verified", e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm outline-none bg-white">
        <option value="">All</option><option value="true">Verified only</option>
      </select>
      {params.toString() && <button onClick={() => router.push("/explore")} className="text-sm text-red-600 hover:underline">Clear</button>}
    </div>
  );
}
