        ...
        <div className="mx-auto max-w-5xl px-5 pt-8 sm:px-6">
          ...
          {types.length > 1 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {types.map((t) => (
                <Link key={t} href={`/region/${district}/${t}`} className="inline-flex items-center rounded-full border border-gray-300 bg-white px-3.5 py-1.5 text-sm capitalize text-gray-700 transition-colors hover:border-green-600 hover:text-green-800">
                  {landLabel(t)}
                </Link>
              ))}
            </div>
        ...
        <div className="mx-auto max-w-5xl px-5 py-8 sm:px-6">
          {markers.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-gray-300 py-16 text-center">
              ...
              <NotifyMe district={name} prompt={`Want land in ${name}? Get notified when it's listed.`} />
            </div>
          ) : (
